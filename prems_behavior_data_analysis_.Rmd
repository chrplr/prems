---
title: "PREMS -- Analyses of ANT cognitive task, balance tests and physiological measurements"
...
write.table(runeffect, "ant_task_reaction_times.csv", sep=",",   row.names = FALSE)
...
runs
ggplot(runs, aes(x=run, y=mean, colour = group), show.legend = FALSE) +  geom_point(position=pd) + geom_line(aes(group=group), position=pd, size=1) +  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1, position=pd, size=1) + ylim(450, 650) + ylab("Decision Time (ms)") +
theme(text = element_text(size = 20),
      axis.text.x = element_text(size=16)) 
ggsave("ant_task_decision_times.svg", device = svglite::svglite)
ggsave("ant_task_decision_times.pdf")
```
## Analysis of Variance 

```{r}
rt <- aov_car(mean_rt ~ group + Error(subject_id/run), data=runeffect)
knitr::kable(nice(rt))
```
...
write.table(runeffect, "ant_task_flanker_cost.csv", sep=",", row.names=FALSE)
...
ggplot(runs2, aes(x=run, y=cost, colour = group), show.legend = FALSE) +  
  geom_point(position=pd) + 
  geom_line(aes(group=group), position=pd, size=1) +
  geom_errorbar(aes(ymin=cost-se.incong, ymax=cost+se.incong), width=.1, position=pd, size=1) + 
  ylim(0, 250) + 
  ylab("Flanker cost (ms)") +
  theme(text = element_text(size = 20),
      axis.text.x = element_text(size=16))
ggsave("ant_task_flanker_cost.pdf")
ggsave("ant_task_flanker_cost.svg", device = svglite::svglite)
...
```{r, echo=F}
physio = read_csv('physio_data_20240421.csv', show_col_types = FALSE)
...
write.table(equill, "balance_test_data.csv", sep=",", row.names=FALSE)
```
